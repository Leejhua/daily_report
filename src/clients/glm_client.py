...
4. 整体工作效率和时间分配"""
    ...
    async def analyze_daily_report_content(self, daily_summary: str, daily_plan: str) -> str:
        ...
        system_prompt = """你是一位专业的日报分析专家。请对用户的日报进行精准分析，重点关注内容的清晰度和表述质量。
...
重点关注日报内容的清晰度分析：
- 工作描述是否具体明确（避免模糊词汇如"处理了一些事情"、"做了相关工作"）
- 进度表述是否量化清晰（避免"差不多"、"基本完成"等模糊表述）
- 问题描述是否详细具体（避免"遇到了问题"等笼统描述）
- 时间安排是否明确（避免"上午"、"下午"等模糊时间）
- 成果产出是否可衡量（避免"有所进展"等无法量化的表述）"""
