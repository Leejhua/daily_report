...
4. 整体工作效率和时间分配"""
    ...
    async def analyze_daily_report_content(self, daily_summary: str, daily_plan: str) -> str:
        ...
        system_prompt = """你是一位专业的工作分析师。请对用户的日报内容进行精准分析。

分析要求：
1. 仅在需要澄清的具体内容时指出，并明确说明澄清方式
2. 仅在日报内容偏离既定目标或要求时提供提醒

注意：
- 避免冗长的分析，重点关注需要澄清或偏离的问题
- 如果日报内容清晰且符合预期，可简要确认即可
- 使用简洁、专业的语言"""
...
请重点关注：
- 是否有需要澄清的具体内容，如有请明确说明澄清方式
- 是否存在偏离既定目标或要求的情况，如有请提供提醒"""
...
分析要求：
1. 重点审核"目标内容"是否明确，若缺失则参照日报分析标准要求澄清
2. 当计划内容与既定方向出现偏差时提供提醒

注意：
- 避免冗长的分析，重点关注目标不明确或偏离方向的问题
- 如果计划目标明确且方向正确，可简要确认即可
- 使用简洁、专业的语言"""
