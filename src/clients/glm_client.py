...
4. 整体工作效率和时间分配"""
    ...
    async def analyze_daily_report_content(self, daily_summary: str, daily_plan: str) -> str:
        ...
        system_prompt = """你是一位专业的工作分析师。请专门对用户的日报内容进行深入分析，重点关注当天的工作完成情况和执行效果。

分析要求：
1. 工作完成情况：分析当天实际完成的工作内容、质量和成果
2. 执行效果评估：评估工作的完成度、质量和效率
3. 问题识别：识别工作中遇到的具体问题和挑战
4. 解决方案分析：分析问题的解决过程和效果
5. 工作亮点：识别当天工作中的亮点和成就
6. 改进建议：针对工作执行提供具体的改进建议

注意：请专注于日报内容本身的分析，不要涉及计划制定、计划一致性等计划相关的分析内容。
请用专业、简洁的语言进行分析，重点突出工作执行的关键发现和建议。"""
...
- 时间分配的合理性"""
    ...
    async def analyze_daily_plan_content(self, daily_plan: str, weekly_plan: str) -> str:
        ...
        system_prompt = """你是一位专业的计划管理专家。请对用户的日计划进行深入分析，重点关注计划的合理性、可执行性和与周计划的一致性。

分析要求：
1. 计划合理性：评估任务量、优先级和时间分配是否合理
2. 一致性分析：分析日计划与周计划的目标一致性
3. 可执行性：评估计划的可实现性和潜在风险
4. 优先级评估：分析任务优先级设置是否合适
5. 改进建议：提供计划优化建议

请用专业、简洁的语言进行分析，重点突出关键发现和建议。"""
